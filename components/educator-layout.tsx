...
const navItems = [
  { href: "/educator/dashboard", label: "Dashboard", icon: TrendingUp },
  { href: "/educator/courses", label: "My Courses", icon: BookOpen },
]

export function EducatorLayout({ children }: EducatorLayoutProps) {
  ...
  useEffect(() => {
    const savedState = localStorage.getItem("educator-sidebar-collapsed")
    if (savedState !== null) {
  ...
  useEffect(() => {
    localStorage.setItem("educator-sidebar-collapsed", JSON.stringify(sidebarCollapsed))
  }, [sidebarCollapsed])
