...
export function LearnerLayout({ children }: LearnerLayoutProps) {
  ...
  useEffect(() => {
    const savedState = localStorage.getItem("learner-sidebar-collapsed")
    if (savedState !== null) {
  ...
  useEffect(() => {
    localStorage.setItem("learner-sidebar-collapsed", JSON.stringify(sidebarCollapsed))
  }, [sidebarCollapsed])
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Sidebar */}
      <aside
        className={`fixed left-0 top-0 h-screen ${sidebarWidth} bg-secondary-background border-r-4 border-border transform transition-all duration-300 ease-in-out z-40 ${
          sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
        }`}
      >
        <div className={`border-b-2 border-border transition-all duration-300 ${
          ...
          <div className={`flex items-center transition-all duration-300 ${
            ...
            >
              <div className={`bg-main border-2 border-border rounded-base flex items-center justify-center shadow-shadow flex-shrink-0 transition-all duration-300 ${
                sidebarCollapsed ? "w-8 h-8" : "w-10 h-10"
              }`}>
                <Brain className={`text-main-foreground transition-all duration-300 ${
                  sidebarCollapsed ? "w-4 h-4" : "w-6 h-6"
                }`} />
              </div>
              <span className={`font-heading text-xl transition-all duration-300 whitespace-nowrap ${
            ...
            <button
              ...
              className={`hidden md:flex items-center justify-center rounded-base border-2 border-border hover:bg-main/10 active:bg-main/20 transition-all duration-300 flex-shrink-0 z-10 relative ${
                sidebarCollapsed ? "w-7 h-7 mt-1" : "w-8 h-8"
              }`}
          ...
          {navItems.map((item) => {
            ...
            return (
              <div key={item.href} className="relative group">
                <Link
                  href={item.href}
                  className={`flex items-center rounded-base transition-all duration-300 relative ${
                    sidebarCollapsed 
                ...
                >
                  ...
                  <span className={`transition-all duration-300 whitespace-nowrap ${
                    sidebarCollapsed ? "opacity-0 w-0 overflow-hidden absolute" : "opacity-100 relative"
                  }`}>
                    {item.label}
                  </span>
        ...
        <div className={`absolute bottom-0 left-0 right-0 border-t-2 border-border transition-all duration-300 ${
          sidebarCollapsed ? "p-2 space-y-1" : "p-4 space-y-2"
        }`}>
          <div className="relative group">
            <Link
              href="/learner/settings"
              className={`flex items-center rounded-base text-foreground hover:bg-main/5 transition-all duration-300 ${
                sidebarCollapsed 
                  ? "justify-center px-0 py-2.5" 
                  : "gap-3 px-4 py-3"
              }`}
            >
              <Settings className={`flex-shrink-0 transition-all duration-300 ${
                sidebarCollapsed ? "w-5 h-5" : "w-5 h-5"
              }`} />
              <span className={`transition-all duration-300 whitespace-nowrap ${
                sidebarCollapsed ? "opacity-0 w-0 overflow-hidden absolute" : "opacity-100 relative"
          ...
          <div className="relative group">
            <button
              ...
              className={`w-full flex items-center rounded-base text-foreground hover:bg-destructive/10 transition-all duration-300 ${
                sidebarCollapsed 
                  ? "justify-center px-0 py-2.5" 
                  : "gap-3 px-4 py-3"
              }`}
